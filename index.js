...
const botUsername = 'BOTRTM_BOt'; // ✅ تم تعيينه يدويًا حسب طلبك
...
function sendMainMenu(chatId, name) {
  const text =
    ...
    `✅ اكتب في أي مكان: <code>@BOTRTM_BOt play</code>\n` +
    `✅ أو: <code>@BOTRTM_BOt play 10</code> لرهان 10 نقاط.\n` +
...
// 🚀 جاهزية البوت
// ==================================================
// اسم البوت تم تحديده يدويًا في الأعلى
...
bot.onText(/^\/(?:board|اللوحة)(?:@\w+)?$/, (msg) => {
  ...
  if (!list.length) {
    return bot.sendMessage(
      msg.chat.id,
      'لا توجد بيانات بعد.\nابدأ أول تحدي عبر @BOTRTM_BOt play'
    );
  }
...
// ==================================================
// Inline Mode — @BOTRTM_BOt play (+ رهان اختياري)
// ==================================================
  ...
  try {
    ...
    if (!q || q === 'play' || q === 'xo' || /^\d+$/.test(q) || /^(?:play|xo)\s+\d+$/.test(q)) {
      ...
      const textX =
        ...
        `⚠️ استخدم البوت الصحيح: @BOTRTM_BOt`;
      const textO =
        `🎮 تحدي XO\n` +
        `⭕ ${fromName} هو اللاعب الأول.\n` +
        betLine +
...
bot.on('chosen_inline_result', async (res) => {
  try {
    ...
    games[gameId] = {
      ...
      bet: bet > 0 ? bet : 0,
      stakeActive: false,
      botUsername: 'BOTRTM_BOt', // ← لدعم التحقق مستقبلاً
    };
  } catch (err) {
...
bot.on('callback_query', async (query) => {
  ...
  // ---------- MENUS ----------
  const menuHandlers = {
    'menu:home': () => sendMainMenu(from.id, user.name),
    'menu:friend': () => 
      `👥 <b>اللعب مع صديق</b>\n` +
      `1️⃣ اكتب: <code>@BOTRTM_BOt play</code> أو <code>@BOTRTM_BOt play 10</code>\n` +
    ...
    'menu:board': () => {
      const list = Object.values(players).sort((a, b) => (b.points || 0) - (a.points || 0));
      if (!list.length) return 'لا توجد بيانات بعد.\nابدأ أول تحدي عبر @BOTRTM_BOt play';
      const top = list.slice(0, 20);
      const lines = top.map((p, i) => `${i + 1}. ${p.name} — ${p.points} نقطة (${getTitle(p)})`);
      return '🌍 التصنيف العالمي:\n' + lines.join('\n');
    },
    'menu:shop': () => '🛍 المتجر',
    'menu:gift': () => 
      '🎁 <b>الهدايا</b>\n' +
    ...
    'menu:help': () => 
      'ℹ️ <b>مختصر XO Bot</b>\n' +
      '• تحدي صديق: @BOTRTM_BOt play أو @BOTRTM_BOt play 10\n' +
  ...
  if (data in menuHandlers) {
    const handler = menuHandlers[data];
    let txt = typeof handler === 'function' ? handler() : handler;
    if (message) {
      let reply_markup = backHomeKeyboard();
      if (data === 'menu:shop') reply_markup = buildShopKeyboard(user);
      if (data === 'menu:bot') {
        reply_markup = {
          inline_keyboard: [
            [
      ...
      await sendMainMenu(from.id, user.name);
    }
    await bot.answerCallbackQuery(id).catch(() => {});
    return;
  }
  ...
  if (data.startsWith('shop:')) {
    ...
    if (action === 'buyExtra') {
      ...
      await bot.answerCallbackQuery(id, { text: `✅ تم شراء ${item.name}.` }).catch(() => {});
      return;
    }
    await bot.answerCallbackQuery(id).catch(() => {});
    return;
  ...
  if (data.startsWith('join:')) {
    ...
    // ✅ التحقق من اسم البوت
    if (game && game.botUsername && game.botUsername !== 'BOTRTM_BOt') {
      await bot.answerCallbackQuery(id, {
        text: `❌ هذا التحدي تم إنشاؤه بواسطة بوت آخر (@${game.botUsername}).\nاستخدم @BOTRTM_BOt لبدء تحدي جديد.`,
        show_alert: true,
      }).catch(() => {});
      return;
    }
  ...
  if (data.startsWith('mv:')) {
    ...
    if (!game || game.status !== 'playing') {
      await bot.answerCallbackQuery(id, { text: '❌ لا توجد لعبة نشطة.' }).catch(() => {});
      return;
    }
  ...
  if (data.startsWith('botlvl:')) {
    ...
    if (String(from.id) !== String(userId)) {
      await bot.answerCallbackQuery(id, { text: '⚠️ هذا الاختيار لصاحب الطلب فقط.' }).catch(() => {});
      return;
    }
    if (!message) {
      await bot.answerCallbackQuery(id).catch(() => {});
  ...
  if (data.startsWith('botmv:')) {
    ...
    if (!game) {
      await bot.answerCallbackQuery(id, { text: '❌ لا توجد لعبة.' }).catch(() => {});
      return;
    }
    ...
    if (from.id !== game.userId) {
      await bot.answerCallbackQuery(id, { text: '⚠️ هذه المباراة ليست لك.' }).catch(() => {});
      return;
    }
    ...
    game.turn = 'X';
    const txt = `🤖 تحدي البوت (${game.level})\nأنت ${skin.icons.X || '❌'} ، البوت ⭕\n🎯 دورك الآن.`;
    await bot.editMessageText(txt, {
      chat_id: message.chat.id,
      message_id: message.message_id,
      reply_markup: buildKeyboard(),
...
console.log('🚀 XO Inline Bot يعمل بـ @BOTRTM_BOt play');